...
class ApprovalTests {
  ...
  static void verify(String response, {Options options = const Options(), String? file, int? line}) {
    try {
      ...
      if (!isFilesMatch) {
        AppLogger.log('Approval Test Failed: ${namer.approved} does not match ${namer.received}');
        throw Exception('Content does not match approved file.');
      } else if (isFilesMatch) {
        AppLogger.good('Approval Test Passed: ${namer.approved} matches ${namer.received}');
      }
